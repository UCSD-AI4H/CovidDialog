...
def setup_train_args():
    ...
    parser.add_argument('--device', default='5,6,7', type=str, required=False, help='设置使用哪些显卡')
    ...
    parser.add_argument('--train_raw_path', default='data/train_med.txt', type=str, required=False, help='原始训练语料')
    parser.add_argument('--train_tokenized_path', default='data/train_med_tokenized.txt', type=str,
                        required=False,
                        help='将原始训练语料tokenize之后的数据的存放位置')
    parser.add_argument('--valid_raw_path', default='data/validate_med.txt', type=str, required=False, help='原始验证语料')
    parser.add_argument('--valid_tokenized_path', default='data/validate_med_tokenized.txt', type=str,
                        required=False,
                        help='将原始验证语料tokenize之后的数据的存放位置')
    ...
    parser.add_argument('--epochs', default=4, type=int, required=False, help='训练的轮次')
    ...
    parser.add_argument('--dialogue_model_output_path', default='dialogue_model/', type=str, required=False,
                        help='对话模型输出路径')
    parser.add_argument('--pretrained_model', default='dialogue_model/med_model_epoch3', type=str, required=False, help='预训练的GPT2模型的路径')
    ...
    parser.add_argument('--train_mmi_tokenized_path', default='data/train_med_mmi_tokenized.txt', type=str,
                        required=False,
                        help='将原始训练语料的每段对话翻转，然后进行tokenize之后的数据的存放位置，用于训练MMI模型')
    parser.add_argument('--valid_mmi_tokenized_path', default='data/validate_med_mmi_tokenized.txt', type=str,
                        required=False,
                        help='将原始验证语料的每段对话翻转，然后进行tokenize之后的数据的存放位置，用于训练MMI模型')
...
#    else:
    valid_data = data.split("\n\n")
    logger.info("there are {} dialogue in raw dataset".format(len(valid_data)))
    with open(args.valid_tokenized_path, "w", encoding="utf-8") as f:
        for dialogue_index, dialogue in enumerate(tqdm(valid_data)):
#            if "\r\n" in data:
#                utterances = dialogue.split("\r\n")
#            else:
            utterances = dialogue.split("\n")
...
#    else:
    test_data = data.split("\n\n")
    logger.info("there are {} dialogue in raw dataset".format(len(test_data)))
    with open(args.test_tokenized_path, "w", encoding="utf-8") as f:
       for dialogue_index, dialogue in enumerate(tqdm(test_data)):
#           if "\r\n" in data:
#               utterances = dialogue.split("\r\n")
#           else:
           utterances = dialogue.split("\n")
           ...
           for dialogue_id in dialogue_ids:
               f.write(str(dialogue_id) + ' ')
           # 最后一条记录不添加换行符
           if dialogue_index < len(test_data) - 1:
               f.write("\n")
    logger.info("finish preprocessing raw data,the result is stored in {}".format(args.test_tokenized_path))
...
#    else:
    valid_data = data.split("\n\n")
    logger.info("there are {} dialogue in raw dataset".format(len(valid_data)))
    with open(args.valid_mmi_tokenized_path, "w", encoding="utf-8") as f:
        for dialogue_index, dialogue in enumerate(tqdm(valid_data)):
#            if "\r\n" in data:
#                utterances = dialogue.split("\r\n")
#            else:
...
#Start Test Set
    ...
    logger.info("tokenizing MMI raw data,raw data path:{}, token output path:{}".format(args.test_raw_path,
                                                                                        args.test_mmi_tokenized_path))
    with open(args.test_raw_path, 'rb') as f:
        data = f.read().decode("utf-8")
#    if "\r\n" in data:
#        test_data = data.split("\r\n\r\n")
#    else:
    test_data = data.split("\n\n")
    logger.info("there are {} dialogue in raw dataset".format(len(test_data)))
    with open(args.test_mmi_tokenized_path, "w", encoding="utf-8") as f:
        for dialogue_index, dialogue in enumerate(tqdm(test_data)):
#            if "\r\n" in data:
#                utterances = dialogue.split("\r\n")
#            else:
...
#                                                                                accuracy))
        ...
        if args.train_mmi:  # 当前训练MMI模型
            model_path = join(args.mmi_model_output_path, 'med_model_epoch{}'.format(epoch + 1))
        else:  # 当前训练对话模型
            model_path = join(args.dialogue_model_output_path, 'med_model_epoch{}'.format(epoch + 1))
